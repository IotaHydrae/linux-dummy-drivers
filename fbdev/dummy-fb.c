...
static void dummy_fb_fillrect(struct fb_info *info,
...
static void dummy_fb_copyarea(struct fb_info *info,
...
static void dummy_fb_imageblit(struct fb_info *info,
...
static void dummy_fb_deferred_io(struct fb_info *info,
...
static int dummy_fb_alloc(struct dummy_fb *dfb)
{
	...
	if (!vmem) {
		pr_err("failed to allocvzaate vmem\n");
		return -ENOMEM;
	}
	...
	fbops->fb_read = dummy_fb_read, fbops->fb_write = dummy_fb_write;
...
static int __init dummy_fb_init(void)
{
	...
	dfb->dev = device_create(dfb->class, NULL, MKDEV(0, 0), NULL,
				 DRV_NAME "dev");
	if (IS_ERR(dfb->dev)) {
